...
async function goPatch(endpoint, dataToStore, toggleBusy, toggleSuccess, toggleError, afterSuccess) {
    if (typeof dataToStore !== "object") {
        console.error("object required to patch a little sumthin' sumthin'");
        return;
    }
    ...
    try {
        ...
        console.log("Going to patch sumthin' at " + endpoint);
        const result = await axios.patch(endpoint2url(endpoint), dataToStore, {
            headers: {
